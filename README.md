# my_namespace.jinja_utils

Ansible collection providing utility filters for working with Jinja2 template strings stored in variables.

## Installation

```bash
ansible-galaxy collection install my_namespace-jinja_utils-1.0.0.tar.gz
```

Or from Galaxy (once published):

```bash
ansible-galaxy collection install my_namespace.jinja_utils
```
